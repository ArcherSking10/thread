...
async function init() { updateNav(); await requireAuth(); showStep(1); }
...
window.submitDetails = async e => {
  ...
  try {
    const order = await createOrder({
      ...
      edition_index: editionIdx, stripe_price_id: priceId,
      photo_url: uploadedPhotoUrl, subject_name: fd.get("subject_name"),
      customer_email: user.email, notes: fd.get("notes")
    });
